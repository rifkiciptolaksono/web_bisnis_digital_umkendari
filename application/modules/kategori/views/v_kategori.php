...
<div class="row">
	<!--kategori baru-->
	<div class="col-lg-4">
		<form name="form_tambah_kategori" id="form-tambah-kategori" method="post">
			<input name="id_kategori" id="id-kategori" type="hidden">
			<div class="form-group">
				<!-- <label>Icon Kategori (url) *icon 76px X 76px</label> -->
				<div class="input-group">
					...
					<span class="input-group-addon"><i class="fa fa-picture-o" id="btn-gambar-pilih" data-toggle="modal" data-target=".bs-gambar-andalan"></i></span>
				</div>
			</div>
			<div class="form-group">
				<label>Nama</label>
				<input id="nama-kategori" name="nama_kategori" type="text" class="form-control square-btn-adjust" title="Nama kategori" required>
			</div>
			<div class="form-group">
				<label>Slug</label>
				<input id="slug-kategori" name="slug_kategori" type="text" class="form-control square-btn-adjust" title="Slug">
			</div>
			<div class="form-group">
				<!-- <label>Induk</label> -->
				<select id="induk-kategori" name="induk_kategori" class="form-control square-btn-adjust auto-width" title="Induk kategori">
					<option value="0">- kategori induk -</option>
				</select>
			</div>
			<div class="form-group">
				<label>Deskripsi</label>
				<textarea id="deskripsi-kategori" name="deskripsi_kategori" class="form-control square-btn-adjust" rows="5" title="Deskripsi kategori"></textarea>
			</div>
			<div class="form-group">
				<button type="reset" class="md-btn md-btn-default btn-sm square-btn-adjust" title="Batal/Reset Form" onclick="form_tambah_kategori_reset()"><li class="fa fa-times"></li> Batal</button>
				<button type="submit" class="md-btn md-btn-primary btn-sm square-btn-adjust" title="Tambah kategori baru"><li class="fa fa-save"></li> Tambah Kategori Baru</button>
			</div>
		</form>
	</div>
	<!--kategori baru end-->
	...
	<div class="col-lg-8" style="border-left:1px solid #f0f0f0;">
		<form name="form_hapus_kategori_massal" id="form-hapus-kategori-massal" method="post">
			...
			<div class="form-group">
				...
				<div style="float:left" class="row col-md-5">
					<div class="input-group">
						...
						<span class="btn btn-default square-btn-adjust input-group-addon" title="Terapkan" onclick="submit('#form-hapus-kategori-massal');">Terapkan</span>
					</div>
				</div>
				<?php }?>
				<div style="float:right" class="row col-md-5">
					<input id="cari-kategori" onkeypress="cari_kategori_()" onchange="cari_kategori_()" onblur="cari_kategori_()" name="cari_kategori" type="text" class="form-control square-btn-adjust" title="Cari kategori">
				</div>
				<br />
			</div>
		</div>
		<div class="table-responsive">
			...
			<table class="uk-table">
				<thead>
					<tr>
						<?php if($this->session->userdata('level')!='user'){?>
						<th width="5%" class="md-bg-grey-100"><input id="check-kategori-all-atas" onclick="check_kategori_all_atas()" type="checkbox"></th>
						<?php }?>
						<th width="35%" class="md-bg-grey-100">Nama</th>
						<th width="35%" class="md-bg-grey-100">Deskripsi</th>
						<th width="25%" class="md-bg-grey-100">Slug</th>
					</tr>
				</thead>
				...
				<tfoot>
					<tr>
						<?php if($this->session->userdata('level')!='user'){?>
						<th class="md-bg-grey-100"><input id="check-kategori-all-bawah" onclick="check_kategori_all_bawah()" type="checkbox"></th>
						<?php }?>
						<th class="md-bg-grey-100">Nama</th>
						<th class="md-bg-grey-100">Deskripsi</th>
						<th class="md-bg-grey-100">Slug</th>
					</tr>
				</tfoot>
			</table>
			<!--tabel kategori end-->
		...
		<div class="block" style="margin-top:10px;">
			<div class="form-group">
				...
				<div style="float:right">
					<i><gedank id="jml-list-bawah">0</gedank> kategori</i>
				</div>
			</div>
		</div>
		</form>
			...
			<div class="row">
				<div class="col-lg-12">
					<form id="form_action_del" action="" method="post">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h3 id="modal_title" class="modal-title">Hapus Kategori</h3>
						</div>
						<div class="modal-body">
...
$(document).ready(function(){
	...
	$('#form-hapus-kategori-massal').submit(function(e){
		e.preventDefault();
		$.ajax({
	        'type': 'POST',
	        'url': '<?php echo site_url("kategori/delkategorimassal");?>',
	        'dataType': 'json',
	        'data': $(this).serialize(),
	        success: function(data){
	        	//console.log(data);
...
function cari_kategori_(){
	console.log($('#cari-kategori').val());
	$.ajax({
        'type': 'POST',
        'url': '<?php echo site_url("kategori/searchkategori");?>',
        'dataType': 'json',
        'data': 'cari_kategori='+$('#cari-kategori').val(),
        success: function(data){
        	//console.log(data);
        	if(data.data!=null){
				isi(data.data);
    ...
    $('#check-kategori-all-bawah').prop('checked', false);
    $('#check-kategori-all-atas').prop('checked', false);
}
...
function check_kategori_all_atas(){
	if(document.getElementById('check-kategori-all-atas').checked==true){
		$('.check-kategori').prop('checked', true);
		$('#check-kategori-all-bawah').prop('checked', true);
	}else{
...
function check_kategori_all_bawah(){
	if(document.getElementById('check-kategori-all-bawah').checked==true){
		$('.check-kategori').prop('checked', true);
		$('#check-kategori-all-atas').prop('checked', true);
	}else{
		$('.check-kategori').prop('checked', false);
		$('#check-kategori-all-atas').prop('checked', false);
	}
...
function set_edit(data){
	...
	$('#nama-kategori').val(data.nm);
	$('#slug-kategori').val(data.slug);
	$('#induk-kategori option[value='+data.id_idk+']').prop('selected', true);
	$('#deskripsi-kategori').val(data.des);
...
function isi(data){
	//console.log(data);
	var user = "<?php echo $this->session->userdata('level');?>";
	if(data.error==0){
		table = '';
		...
		$.each(data.kategori, function(i,n){
			...
			table += '<tr id="hapus-row-kategori-'+n['id_kategori']+'" onmouseover="row_actions_over(\''+n['id_kategori']+'\')" onmouseout="row_actions_blur(\''+n['id_kategori']+'\')">';
				
				if(user!='user'){
					table +='<td>';
					if(n['id_kategori']!='1'){
						table +='<input name="id_hapus_kategori_massal_['+n['id_kategori']+']" class="check-kategori" type="checkbox" value="'+n['id_kategori']+'">';
					}
					table +='</td>';
				}
				table +='<td><p>';
					...
					+'<div class="row-actions" id="row-actions-'+n['id_kategori']+'" style="display:none;">'
						...
						if(n['id_kategori']!='1'){
							if(penulis=='administrator'){
								table +='<span class="hapus"> | <a title="Hapus '+n['kategori']+'" data-toggle="modal" data-target=".bs-hapus-" onclick="set_delete({id:\''+n['id_kategori']+'\',nm:\''+n['kategori']+'\',act:\''+url+'kategori/delkategori\',element:\'hapus-row-kategori-\'})">Hapus</a></span>'
							}
						}
						table +='<span class="tampilkan"> | <a target="blank" href="'+url+'home/kategori/'+n['id_kategori']+'/'+n['slug']+'">Tampilkan</a></span>'
					+'</div>'
				+'</td>'
				+'<td>'+n['deskripsi']+'</td>'
				+'<td>'+n['slug']+'</td>'
        ...
        success: function(data){
			...
			$.each(data.data.kategori, function(i,n){
				...
				table += '<tr id="hapus-row-kategori-'+n['id_kategori']+'" onmouseover="row_actions_over(\''+n['id_kategori']+'\')" onmouseout="row_actions_blur(\''+n['id_kategori']+'\')">'
					...
					table+=n['kategori']+'</p>'
						+'<br id="br-'+n['id_kategori']+'">'
						+'<div class="row-actions" id="row-actions-'+n['id_kategori']+'">'
							+'<span class="sunting"><a onclick="set_edit({'
								+'id:\''+n['id_kategori']
								+'\',nm:\''+n['kategori']
								+'\',slug:\''+n['slug']
							...
							if(n['id_kategori']!='1'){
								if(penulis=='administrator'){
									table +='<span class="hapus"> | <a title="Hapus '+n['kategori']+'" data-toggle="modal" data-target=".bs-hapus" onclick="set_del({id:\''+n['id_kategori']+'\',nm:\''+n['kategori']+'\',act:\''+url+'kategori/delkategori\',element:\'hapus-row-kategori-\'})">Hapus</a></span>'
								}
							}
							table +='<span class="tampilkan"> | <a target="blank" href="'+url+'lihat/kategori/'+n['slug']+'#li-'+n['id_kategori']+'">Tampilkan</a></span>'
						+'</div>'
					+'</td>'
					+'<td>'+n['deskripsi']+'</td>'
					+'<td>'+n['slug']+'</td>'
			...
			$(table).insertAfter("#hapus-row-kategori-"+id);
