...
$ses = array();
		if($this->session->userdata('lang')==''){$ses['lang'] = 'id';}
		if($this->session->userdata('clang')==''){$this->language('id');}
	...
	function berita() {
		...
		if($berita!=null&&$this->uri->segment(3)!=''&&$this->uri->segment(3)!='page'){
			...
			$cont .= '<div class="berita-detail">';
				...
				$cont .= '		<a target="blank" href="'.site_url('home/pdf/berita/'.$row['id_tulisan']).'"><i class="fa fa-file-o"></i></a> ';
				$cont .= '		<a target="blank" href="'.site_url('home/word/berita/'.$row['id_tulisan']).'"><i class="fa fa-file-text-o"></i></a> ';
				$cont .= '	</span>';
				...
				foreach($tul_lain as $row2){
					...
					$cont .= '			<h4><a class="related_title" href="'.site_url().'home/berita/'.$row2['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},.]/", "_",str_replace(' ','-',strtolower($row2['judul_'.$this->session->userdata('lang')]))).'"> '.ucfirst($row2['judul_'.$this->session->userdata('lang')]).' </a></h4>';
				...
				if($row['status_komentar']=='open'){
					$cont .= '<div class="komentar_user">';
						...
						$cont .= '	<h3> '.clang('Leave a Comment').' </h3>';
						...
						if($this->session->userdata('id_pengguna')==''){
							...
							$cont .= '			<label> '.clang('Name').' </label>';
							$cont .= '			<input class="form-control" type="text" placeholder="'.clang('Name').'" name="nama_komentar" id="nama-komentar">';
							$cont .= '		</div>';
							$cont .= '		<div class="form-group">';
							$cont .= '			<label> '.clang('Email').' </label>';
							$cont .= '			<input class="form-control" type="email" placeholder="'.clang('Email').'" name="email_komentar" id="email-komentar">';
							$cont .= '		</div>';
						}
						$cont .= '		<div class="form-group">';
						$cont .= '			<label> '.clang('Comment').' </label>';
			...
			$cont .= '<ol class="breadcrumb">';
	        $cont .= '    <li><a href="'.site_url('').'">'.clang('home').'</a></li>';
	        $cont .= '    <li class="active">'.clang('News').'</li>';
	        $cont .= '</ol>';
	        ...
	        foreach ($berita as $row) {
				...
				if($row['gambar_andalan']!=''){
					...
					$cont .= '	<div class="row">';
					$cont .= '		<div class="col-lg-3">';
					$cont .= '			<img src="'.$this->path().'/thumb/'.$row['gambar_andalan'].'" style="width:100%;">';
					$cont .= '		</div>';
	...
	function pengumuman() {
		...
		if($berita!=null&&$this->uri->segment(3)!=''&&$this->uri->segment(3)!='page'){
	        ...
	        foreach ($berita as $row) {
				...
				$tgl2 = explode('-',$tgl[0]);
				$cont .= '<div class="berita-detail" style="padding-bottom:10px; border-bottom:1px solid #EEE;">';
				$cont .= '	<h3 class="title-berita-detail"><a href="'.site_url().'home/pengumuman/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},.]/", "_",str_replace(' ','-',strtolower($row['judul_'.$this->session->userdata('lang')]))).'"> '.ucfirst($row['judul_'.$this->session->userdata('lang')]).' </a></h3>';
				$cont .= '	<div class="row">';
				$cont .= '		<div class="col-lg-12">';
				$cont .= '			<span class="tgl-berita-terpopuler">'.clang('Write on').' <i class="fa fa-clock-o"></i> '.hari(date('N',mktime(0,0,0,$tgl2[1],$tgl2[2],$tgl2[0]))).', '.$tgl2[2].' '.bln($tgl2[1]).' '.$tgl2[0].' <i class="fa fa-user"></i> '.clang('by').': '.$row['nm_dp'].'</span>';
				$cont .= '			<p>'.substr(strip_tags($row['tulisan_'.$this->session->userdata('lang')]),0,200).'...';
	...
	function event() {
		...
		if($berita!=null&&$this->uri->segment(3)!=''&&$this->uri->segment(3)!='page'){
			...
			$tgl = explode(' ',$row['tgl_tulisan']);
			$tgl2 = explode('-',$tgl[0]);
			if(isset($row['latitude'])&&$row['latitude']!=''){$latitude = $row['latitude'];}else{$latitude = '';}
			if(isset($row['longitude'])&&$row['longitude']!=''){$longitude = $row['longitude'];}else{$longitude = '';}
	...
	function kategori(){
		...
		if($kategori!=null&&$this->uri->segment(3)!=''){
			...
			$cont .= '<ol class="breadcrumb">';
	        $cont .= '    <li><a href="'.site_url('').'">'.clang('home').'</a></li>';
	        $cont .= '    <li><a href="'.site_url().'home/kategori">Kategori</a></li>';
			...
			foreach ($tulisan as $row) {
				...
				$tgl2 = explode('-',$tgl[0]);
				$cont .= '<div class="berita-detail" style="padding-bottom:10px; border-bottom:1px solid #EEE;">';
				$cont .= '	<h3 class="title-berita-detail"><a href="'.site_url().'home/'.$row['slug'].'/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},.]/", "_",str_replace(' ','-',strtolower($row['judul_'.$this->session->userdata('lang')]))).'"> '.ucfirst($row['judul_'.$this->session->userdata('lang')]).' </a></h3>';
				$cont .= '	<div class="row">';
				$cont .= '		<div class="col-lg-12">';
				$cont .= '			<span class="tgl-berita-terpopuler">'.clang('Write on').' <i class="fa fa-clock-o"></i> '.hari(date('N',mktime(0,0,0,$tgl2[1],$tgl2[2],$tgl2[0]))).', '.$tgl2[2].' '.bln($tgl2[1]).' '.$tgl2[0].' <i class="fa fa-user"></i> '.clang('by').': '.$row['nm_dp'].'</span>';
				$cont .= '			<p>'.substr(strip_tags($row['tulisan_'.$this->session->userdata('lang')]),0,200).'...';
	...
	function set_komentar($row,$parent,$i){
		$bln=array('01'=>'Januari','02'=>'Februari','03'=>'Maret','04'=>'April','05'=>'Mei'
		,'06'=>'Juni','07'=>'Juli','08'=>'Agustus','09'=>'September','10'=>'Oktober'
		,'11'=>'November','12'=>'Desember');

		$hari=array('1'=>'Senin','2'=>'Selasa','3'=>'Rabu','4'=>'Kamis','5'=>'Jumat'
		,'6'=>'Sabtu','7'=>'Minggu');
		$komentar = "";
	...
	function errors(){
		...
		$cont .= '<div class="berita-content">';
		$cont .= '	<div class="view-content-berita" style="min-height:200px;">';
		$cont .= '	<div class="wrap">';
		$cont .= '		<div class="row">';
		$cont .= '			<div class="col-lg-12 content-center-position">';
		$cont .= '				<div class="listing berita-auto listing-black">';
		$cont .= '					<div class="first-auto item-black" style="padding-bottom:80px">';
		$cont .= '						<div class="row">';
		$cont .= '							<div class="col-lg-12 text-center">';
		$cont .= '								<h2>Whoops <small>this page could not be found</small></h2>';
		$cont .= '							</div>';
		$cont .= '						</div>';

		$cont .= '						<div class="row">';
		$cont .= '							<div class="col-lg-12 text-center">';
		$cont .= '								<h1 class="title-berita">404</h1>';
		$cont .= '								<h2>Were not sure what happened,<br>but we do know this page doesnt exist.</h2>';
		$cont .= '							</div>';
		$cont .= '						</div>';
		$cont .= '					</div>';
		$cont .= '				</div>';
