...
if(!function_exists('get_bahasa')) {
	function get_bahasa(){
		...
		if(CI()->session->userdata('lang')=='id'){$lang .= '<span class="flag-ID"></span>&nbspID&nbsp&nbsp';}
		if(CI()->session->userdata('lang')=='eng'){$lang .= '<span class="flag-_england"></span>&nbspEND&nbsp&nbsp';}
		if(CI()->session->userdata('lang')=='ae'){$lang .= '<span class="flag-AE"></span>&nbspAE&nbsp&nbsp';}
		$lang .= '<span class="caret"></span>';
		$lang .= '</button>';
		$lang .= '<ul class="dropdown-menu">';
		if(CI()->session->userdata('lang')!='id'){$lang .= '<li><a href="'.site_url('home/language/id').'"><span class="flag-ID"></span>&nbspID</a></li>';}
		if(CI()->session->userdata('lang')!='eng'){$lang .= '<li><a href="'.site_url('home/language/eng').'"><span class="flag-_england"></span>&nbspEND</a></li>';}
		if(CI()->session->userdata('lang')!='ae'){$lang .= '<li><a href="'.site_url('home/language/ae').'"><span class="flag-AE"></span>&nbspAE</a></li>';}
		$lang .= '</ul>';
		$lang .= '</div>';
		return $lang;
	}
...
if(!function_exists('get_book_opac')) {
	function get_book_opac() {
		...
		foreach($data as $row){
			...
			$slider .= '				<h3><a target="blank" href="http://opac.technophoriajogja.com/index.php/opac/detail/'.$row['biblio_id'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['biblio_title']))).'" title="'.$row['biblio_title'].'"> ';$row['biblio_title'].' </a></h3>';
	...
	function get_slideshow($indicator=null,$id=null) {
		...
		if(@$slide['slide']){
			...
			foreach($slide['slide'] as $row){
				if($no==1){ $first = 'active'; }else{ $first = ''; }
...
if(!function_exists('get_slider_post')) {
	function get_slider_post($label='berita',$limit=5,$indicator=null,$id=null,$screen='',$l='6',$r='6') {
		...
		foreach($berita['tulisan_'.CI()->session->userdata('lang')] as $row){
			...
			if($row['gambar_andalan']!=''){ $img = CI()->path().'/'.$row['gambar_andalan']; }else{ $img = base_url('assets/img/web/no_image.png'); }
			...
			if($screen==''){
				...
				$slider .= '			<h1><a href="'.site_url().'home/berita/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['judul_'.CI()->session->userdata('lang')]))).'" title="'.$row['judul_'.CI()->session->userdata('lang')].'">'.ucfirst(substr($row['judul_'.CI()->session->userdata('lang')],0,20));
										if(strlen($row['judul_'.CI()->session->userdata('lang')])>20){$slider .= '...';}
				$slider .= '			</a></h1>';
...
if(!function_exists('get_slider_post_2')) {
	function get_slider_post_2($label = 'berita', $limit = 5) {
		...
		foreach($berita['tulisan_'.CI()->session->userdata('lang')] as $row){
			...
			$slider .= '			<div class="ch-info-2">';
			$slider .= '				<h3><a href="'.site_url().'home/'.$label.'/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['judul_'.CI()->session->userdata('lang')]))).'" title="'.$row['judul_'.CI()->session->userdata('lang')].'"> ';$row['judul_'.CI()->session->userdata('lang')].' </a></h3>';
...
if(!function_exists('get_slider_post_4')) {
	function get_slider_post_4($tipe = 'album', $limit = 5) {
		...
		foreach($berita as $row){
			if($row['gambar_andalan']!=''){ $img = base_url('assets/img/album').'/'.$row['id_tulisan'].'/'.$row['gambar_andalan']; }else{ $img = base_url('assets/img/web/no_image.png'); }
			$slider .= '<div class="recent-work-item">';
			$slider .= '	<div class="text-center" style="margin:0;">';
			$slider .= '		<div class="ch-item-2" style="background-image: url('.$img.');background-position: center;background-size: cover;">';
			$slider .= '			<div class="ch-info-2">';
			$slider .= '				<h3><a href="'.site_url().'home/'.$tipe.'/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['judul_'.CI()->session->userdata('lang')]))).'" title="'.$row['judul_'.CI()->session->userdata('lang')].'"> ';$row['judul_'.CI()->session->userdata('lang')].' </a></h3>';
...
if(!function_exists('list_tab_post')) {
	function list_tab_post($kategori='berita-1',$limit=5,$mode='img') {
		...
		if(@$berita['tulisan_'.CI()->session->userdata('lang')]&&$berita['tulisan_'.CI()->session->userdata('lang')]!=null){
		 	if($mode=='img'){
		 		foreach($berita['tulisan_'.CI()->session->userdata('lang')] as $row){
					...
					$tgl2 = explode('-',$tgl[0]);
					if($no==1){ $active = 'active'; }else{ $active = ''; }
					if($row['gambar_andalan']!=''){ $img = CI()->path().'/thumb/'.$row['gambar_andalan']; }else{ $img = base_url('assets/img/web/no_image.png'); }
					...
					$tab .=	'	<h2><a href="'.site_url().'home/'.$link.'/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['judul_'.CI()->session->userdata('lang')]))).'" title="'.$row['judul_'.CI()->session->userdata('lang')].'">'.ucfirst($row['judul_'.CI()->session->userdata('lang')]).'</a></h2>';
					$tab .= '	<div class="box-img-berita-umum">';
					$tab .= '		<img src="'.$img.'" alt="img">';
					$tab .= '	</div>';
					...
					$tab .= '		<span class="tgl-berita-terpopuler"><i class="fa fa-clock-o"></i> '.hari(date('N',mktime(0,0,0,$tgl2[1],$tgl2[2],$tgl2[0]))).', '.$tgl2[2].' '.bln($tgl2[1]).' '.$tgl2[0].' <i class="fa fa-user"></i> '.clang('by').': '.$row['nm_dp'].'</span>';
					$tab .= '	</div>';
					$tab .= '</div>';
					$no++;
				}
		...
		$tab .= '	</div>';
		return $tab;
		//echo'<textarea>';print_r($berita);echo'</textarea>';
	}
}
...
if(!function_exists('list_tab_post_2')) {
	function list_tab_post_2($label='pengumuman',$limit=5,$mode='img') {
		...
		$tab .= '	<div class="text-right" style="margin-bottom:10px;margin-right:10px;">';
		$tab .= '		<a class="btn btn-sm btn-info" href="'.site_url('home/'.$label).'"><b>'.ucfirst($berita['label'][0]['kategori']).' '.clang('Other').' </b><i class="fa fa-arrow-right"></i></a>';
		$tab .= '	</div>';
		return $tab;
	}
}
...
if(!function_exists('list_post')) {
	function list_post($label = 'berita', $limit = 5, $order='a.id_tulisan desc', $mode=null) {
		...
		foreach($berita['tulisan_'.CI()->session->userdata('lang')] as $row){
			...
			if($row['gambar_andalan']!=''){ $img = CI()->path().'/thumb/'.$row['gambar_andalan']; }else{ $img = ''; }
...
if(!function_exists('get_thumbnail_album')) {
	function get_thumbnail_album($limit) {
		...
		$album_ = '';
		if($album!=null){
			if($album[0]['gambar_andalan']!=''){ $img = base_url('assets/img/album').'/'.$album[0]['id_tulisan'].'/'.$album[0]['gambar_andalan']; }else{ $img = base_url('assets/img/web/no_image.png'); }
...
if(!function_exists('get_list_page')) {
	function get_list_page() {
		...
		$halaman .= '	<h4 class="title-widget"><span>HALAMAN TERKAIT</span><hr/></h4>';
		$halaman .= '	<ul class="box-list-halaman">';
		$no=1;
		foreach($q2 as $row2){
			if($row2['id_tulisan']==CI()->uri->segment(3)){ $active = 'active'; }else{ $active = ''; }
...
if(!function_exists('get_widget_3')){
	function get_widget_3($class='container-footer',$bg='bg_3'){
		...
		if($exc->num_rows() != '0'){
			...
			foreach($exc->result() as $row){
				...
				if($row->widget == '11'){
					...
					echo'	</div>';
					echo'</div>';
					if($class=='container-footer'){
						echo'<div class="container-footer '.$bg.'">';
					...
					get_textarea($row->nama_widget,$row->content_widget,1);
					//echo'	</div>';
					echo'</div>';
				}
				if($row->widget == '12'){
	...
	function get_textarea($judul = '',$content = '',$a = 0) {
		...
		if(CI()->config->config['blogfb']!=''||CI()->config->config['blogtw']!=''||CI()->config->config['bloggp']!=''){
			$textarea .= '<div id="socialicons" class="hidden-phone" style="margin:0px;">';
				if(CI()->config->config['blogfb']!=''){ $textarea .= '<a id="social_facebook" target="blank" class="social_active" href="'.CI()->config->config['blogfb'].'" title="Visit facebook page"><span class="da-animate da-slideFromBottom" style="display: inline-block;"></span></a>';}
				if(CI()->config->config['bloggp']!=''){ $textarea .= '<a id="social_google_plus" target="blank" class="social_active" href="'.CI()->config->config['bloggp'].'" title="Visit google_plus page"><span class="da-animate da-slideFromLeft" style="display: block;"></span></a>';}
				if(CI()->config->config['blogtw']!=''){ $textarea .= '<a id="social_twitter" target="blank" class="social_active" href="'.CI()->config->config['blogtw'].'" title="Visit twitter page"><span class="da-animate da-slideFromRight" style="display: block;"></span></a>';}
			$textarea .= '</div>';
		}
		$textarea .= '	</div>';
		$textarea .= '</div>';
...
if(!function_exists('get_tautan')) {
	function get_tautan($jml=3,$link='link') {
		...
		foreach ($tautan as $row) {
			...
			$tautan_ .='				<p><a href="'.$row['slug'].'" target="blank"> Detail </a></p>';
			$tautan_ .='			</div>';
			$tautan_ .='		</div>';
			$tautan_ .='	</div>';
			$tautan_ .='</div>';
...
if(!function_exists('get_menu')){
	...
	function set_menu($row,$parent,$i,$j,$jenis,$animate,$warna){
      ...
      if(isset($row['id_kategori'])&&isset($parent[$row['id_kategori']])&&$parent[$row['id_kategori']]!=null&&$jenis=='dropdown'){
        ...
        $menu.='<li class="'.$class.' '.$li_warna.'">
          <a class="dropdown-toggle" data-toggle="dropdown" data-target="#" href="#">'.ucfirst($row['kategori']).' '.$simbol.'</a>
...
if(!function_exists('get_menu_atas')){
	function get_menu_atas() {
	    ...
	    $menu .= '<li><a href="'.site_url('login').'"><i class="fa fa-sign-in"></i> '.clang('Login').'</a></li>';
